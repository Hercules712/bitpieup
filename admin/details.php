...
session_start();
	...
	if(isset($_POST['wallet_btn'])){
		...
		$query = mysqli_query($con, $update);
			if($query){
				...
				$ok = 0;
				//$message = " Wallet Update Unsuccessful";
				header('location: members.php?wallet=failed');
			}
	...
	if(isset($_POST['invest_btn'])){
		...
		if($query){
			//Update the wallet with the Balance
			$update = "UPDATE wallet SET balance = '{$invest_balance}' WHERE account = '{$account}'";
				$ok = 1;
				...
				header('location: members.php?invest=failed');
			}
		...
		<div class="row">
			<div class="col-sm-4">
				<div style="margin-top: 10px">
					...
					<h4>Account: <?php echo $bit_acc_number; ?></h4><hr>
				...
				$get = "SELECT * FROM investment WHERE account = '{$bit_acc_number}'";
				$result = mysqli_query($con, $get);
				$row = mysqli_num_rows($result);
										...
										<form role="form" method="POST" action="details.php" >
											...
											<div class="form-group">
											  <div class="input-group"> 
											    ...
											    <input name="wallet" type="text" class="form-control form_element" placeholder="Enter Amount" required>
						                      </div>						
											 
						                    </div>
												  <input type="hidden" name="account" value="<?php echo $bit_acc_number; ?>"> 
										
											<hr/>
											<input type="submit" name="wallet_btn" class="btn btn-green btn-success btn-block auth_button" value="Credit/Debit Wallet">
										</form>	
										<hr/>
					...
					<div class="col-sm-6">
						<div style="margin-right:auto; margin-left:auto; max-width:350px; margin-top:30px; margin-bottom:90px;">  
						      ...
						      <div class="panel panel-default">
								...
								<hr/>	  
								<?php
										    	$select = "SELECT * FROM wallet WHERE account = '{$bit_acc_number}'";
										    ...
										    ?>
										    <p>Current Wallet Balance: <span id="wallet" style="color: red;"><?php echo $u['balance']; ?></span></p>
								   <p style="color:red; text-align:center;">
								   	<?php if(isset( $error_msg)){echo $error_msg;} ?>
										...
										<form role="form" method="POST" action="details.php" >
											...
											<div class="form-group">
											  <div class="input-group"> 
											    ...
											    <input name="invest" id="invest" type="text" class="form-control form_element" placeholder="Enter Investment Amount" required onblur="calc()">
						                      </div>						
						                    ...
						                    <div class="form-group">
											  <div class="input-group"> 
											    ...
											    <input name="balance" id="ballance" type="text" class="form-control form_element" placeholder="Enter Balance">
						                      </div>						
											...
											<hr/>
											<input type="submit" name="invest_btn" class="btn btn-green btn-success btn-block auth_button" value="Credit/Debit Investment">
										</form>	
										<hr/>
